    ...
    for i, ex in enumerate(episode):
        ...
        if get_images:
            ...
            if output_dir is not None:
                ...
                # Save image with episode_id and step_id in filename
                img_path = os.path.join(img_dir, f'{episode_id}_{step_id}.png')
                image.save(img_path)
                output_ep["image_path"] = img_path
            else:
                output_ep["image"] = list(image)
...
def fetch_episode(dataset_name, data_split, get_images, get_annotations, get_actions, output_dir=None):
    ...
    for d in tqdm(dataset):
        ...
        if episode_id is None:
            ...
            try:
                ...
                output = parse_episode(episode, get_images=get_images, get_annotations=get_annotations, 
                                    get_actions=get_actions, output_dir=output_dir, 
                                    episode_id=episode_id, split=curr_split)
...
def parse_args():
    ...
    parser.add_argument("--split_file", type=str, default="splits/general_texts_splits.json")
    parser.add_argument('--output_dir', type=str, default='data/general')
...
if __name__ == '__main__':
    ...
    args = parse_args()
    for splits in ['general', 'google_apps', 'install', 'single', 'web_shopping']:
        args.dataset = splits
        args.output_dir = f'data/{splits}'
        args.split_file = f'splits/{splits}_texts_splits.json'
        ...
        with open(f"{args.output_dir}/train.obj", "wb") as wp:
            pickle.dump(all_parsed_episode["train"],wp)
        with open(f"{args.output_dir}/val.obj", "wb") as wp:
            pickle.dump(all_parsed_episode["val"],wp)
        with open(f"{args.output_dir}/test.obj", "wb") as wp:
